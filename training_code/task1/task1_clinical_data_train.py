...
df = pd.read_csv("/mnt/SSD3/xulin/phd_research/chimera/code/train_test_split/seed3/task1/train_dataset_new.csv")
df.replace(["x", "unknown"], np.nan, inplace=True)  # np.nan  "missing"
df["tertiary_gleason"] = df["tertiary_gleason"].fillna(2)

X = df.drop(columns=["patient_id", "BCR", "time_to_follow-up/BCR", "BCR_PSA", 'label'])
# y_train = df["label"].astype(int)
train_id = df['patient_id'].tolist()
event = df['BCR'].tolist()
converted_event = [1 if x == 0 else 0 for x in event]
survival_time = df['time_to_follow-up/BCR'].tolist()
label = df['label'].tolist()

# test csv
df_test = pd.read_csv("/mnt/SSD3/xulin/phd_research/chimera/code/train_test_split/seed3/task1/test_dataset.csv")
df_test.replace(["x", "unknown"], np.nan, inplace=True)
df_test["tertiary_gleason"] = df_test["tertiary_gleason"].fillna(2)

test_feat = df_test.drop(columns=["patient_id", "BCR", "time_to_follow-up/BCR", "BCR_PSA", 'label'])
# y_test = df_test["BRS_binary"].astype(int)
test_id = df_test['patient_id'].tolist()
event_test = df_test['BCR'].tolist()
converted_event_test = [1 if x == 0 else 0 for x in event_test]
survival_time_test = df_test['time_to_follow-up/BCR'].tolist()
...
categorical_features = ['pT_stage', 'positive_lymph_nodes', 'capsular_penetration',
...
plt.subplot(1, 2, 1)
plt.plot(epochs, train_loss_lst, label="Train Loss", color="tab:red", marker="o")
plt.xlabel("Epoch")
plt.ylabel("Loss")
...
plt.subplot(1, 2, 2)
plt.plot(epochs, train_cindex_lst, label="Train C-index", color="tab:blue", marker="o")
plt.plot(epochs, test_cindex_lst, label="Test C-index", color="tab:green", marker="s")
plt.xlabel("Epoch")
plt.ylabel("C-index")
