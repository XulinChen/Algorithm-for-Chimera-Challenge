...
def load_json_as_row(json_path: str) -> dict:
    ...
    def norm(v):
        ...
        if (isinstance(v, (int, float)) and v == -1) or (isinstance(v, str) and v.strip() == "-1"):
            return np.nan
        return v
...
def clinical_main(json_name, model_dir):
    ...
    trained_feature_cols = numerical_features + categorical_features
    ...
    df_one = align_types_to_preprocessor(
        df_one, preprocessor, numerical_features, categorical_features
    )
    ...
    state = torch.load(model_dir / "051.pt", map_location=device)
